...
# merged.set_index("Period", inplace=True)
#
# # Show how many rows per regime
# print("📊 Regime counts after merge:")
# print(merged["regime"].value_counts())
#
# # Objective function
# def negative_sharpe(weights, mean_returns, cov_matrix, risk_free=0.0):
#     risky_len = len(mean_returns)
#     risky_weights = weights[:risky_len]
#     port_return = np.dot(risky_weights, mean_returns)
#     port_vol = np.sqrt(np.dot(risky_weights.T, np.dot(cov_matrix, risky_weights)))
#     if port_vol == 0:
#         return np.inf
...
#
#     if regime == "Mixed":
#         print("\n🧪 Checking missing data for 'Mixed':")
#         print(subset[risky_assets].isnull().sum())
#         print("🧮 Row count before fill/dropna:", len(subset))
#
#     # Forward fill missing values to reduce data loss
#     subset_risky = subset[risky_assets].ffill().dropna()
#
#     if len(subset_risky) < 2:
#         print(f"⚠️ Skipping regime {regime}: not enough usable data after dropna.")
#         continue
#
#     if regime == "Mixed":
#         print("🧮 Row count after dropna (ffilled):", len(subset_risky))
#
#     mean_returns = subset_risky.mean()
#     cov_matrix = subset_risky.cov()
#
...
#         print(f"❌ Optimizer crashed for {regime}: {e}")
#
# # Save results
# if optimal_allocations:
#     df_opt = pd.DataFrame(optimal_allocations).T
